...
void    rec_print(Node *tmp, int h)
{
    if (tmp == NULL)
        return ;
    rec_print(tmp->right, h + 5);
    printf("%*d\n", h, tmp->key);
    rec_print(tmp->left, h + 5);
}
...
int lvl(Node *node, data_type key)
{
    int q = 0;
        while (!node_is_leaf(node)){
        if (key < node->key){
    ...
    if (node_is_leaf(node) && node->key == key){
        return q;
    } else printf("error\n");
}

void leafscheck_rec(Node *node, Node *mode, int* vc)
{

    if (node != NULL){
        if (node_is_leaf(node)){
            vc = (int *)realloc(vc, (vc[0] + 1) * sizeof(int));

            vc[0]++;
            vc[vc[0]] = lvl(mode, node->key);

                
        }
        leafscheck_rec(node->left, mode, vc);
        leafscheck_rec(node->right, mode, vc);
   }
...
void leafscheck (Bintree *tree)
{   

    int* vc = (int *)malloc(sizeof(int));
    vc[0] = 0;
    leafscheck_rec(tree->root, tree->root, vc);
    if (vc[0] == 0){
